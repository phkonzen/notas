...
\begin{defn}\normalfont{(Iteração Globalmente Convergente.)}
  Um método iterativo
  \begin{gather}
...
\begin{defn}\normalfont{(Ordem de convergência.)}
  Seja $\left(x^{(k)}\right)_{k=1}^\infty$ uma sequência que converge para $x^*$, com $x^{(k)}-x^*\neq 0$ para todo $k$. Dizemos que $\left(x^{(k)}\right)_{k=1}^\infty$ converge para $x^*$ com ordem $\alpha>0$ e com constante de erro assintótica $C>0$, quando
  \begin{equation}
...
\begin{teo}\label{cap_eq1d_sec_bissec:teo:convp}
  Seja $f:[a, b]\to\mathbb{R}$ função contínua com $f(a)\cdot f(b) < 0$. Então, o Método da Bisseção é globalmente converge.
\end{teo}
\begin{dem}
...
Dada uma função contínua e estritamente monótona\footnote{Função estritamente crescente ou estritamente decrescente, exclusivamente.} $f:[a, b]\to\mathbb{R}$ com $f(a)\cdot f(b) < 0$, temos que o Método da Bisseção converge para o zero de $f$ em $[a, b]$.
...
\begin{obs}(\normalfont{Estimativa de Exatidão})
...
\begin{obs}(\normalfont{Ordem de Convergência Linear})
...
\begin{teo}(\normalfont{Teorema do Ponto Fixo})\label{cap_eq1d_sec_pfixo:teo:pfixo}
