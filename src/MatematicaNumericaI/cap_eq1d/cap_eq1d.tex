...
Dada uma função contínua $f:[a, b]\to\mathbb{R}$ com $f(a)\cdot f(b) < 0$, temos que o método da bisseção converge para um zero de $f$ no intervalo $(a, b)$.

\begin{teo}\label{cap_eq1d_sec_bissec:teo:bissec}
  Seja $f:[a, b]\to\mathbb{R}$ função contínua com $f(a)\cdot f(b) < 0$. Então, o método da bisseção converge para um zero de $f$.
\end{teo}
\begin{dem}
  Seja $(x^{(k)})_{k=1}^\infty$ a sequência de aproximações\footnote{Caso, $f\left(a^{(k)}\right)=0$ ou $f\left(a^{(b)}\right)=0$, então assumimos que $a^{(k+i)} = a^{(k)}$ ou $b^{(k+i)} = b^{(k)}$, conforme o caso, para $i=1,2,3,\ldots$.} do método da bisseção com aproximação inicial $x^{(1)}\in [a, b]$. Por construção, temos
  \begin{align}
  ...
  \begin{align}
    \lim_{k\to\infty} \left|x^{(k)}-x^{(k-1)}\right| &= \lim_{k\to\infty} b^{(k)}-a^{(k-1)}\\
                                                     &= 0.
  \end{align}
  Da continuidade de $f$ e para $k$ suficientemente grande, temos que $f(x)=0$ $\forall x\in \left[a^{(k)}, b^{(k)}\right]$ ou $f$ tem um único zero em $\left[a^{(k)}, b^{(k)}\right]$. No primeiro caso, o método converge neste passo. Noutro caso, denotamos $x^*$ o zero de $f$ em $\left[a^{(k)}, b^{(k)}\right]$, i.e.
  \begin{equation}
    f\left(x^*\right) = 0.
  \end{equation}
  Para $l \leq k$, temos
  \begin{align}
    |x^{(l)} - x^{*}| &\leq \frac{b^{(l)}-a^{(l)}}{2}\\
                      &\leq \frac{b^{(l-1)}-a^{(l-1)}}{2^2}\\
                      &\vdots \\
...
\begin{ex}\label{cap_eq1d_sec_bissec:ex:bis_conv}
  No Exemplo \ref{cap_eq1d_sec_bissec:ex:bis_exec} aplicamos o método da bisseção para a função
\begin{equation}
  f(x) = \sen^2\left(x+\frac{\pi}{4}\right) - x^3 + \frac{\pi}{4}x^2 + \frac{5\pi^2}{16}x + \frac{3\pi^3}{64}.
\end{equation}
no intervalo $(2, 3)$. Observando os resultados mostrados na Tabela \ref{cap_eq1d_sec_bissec:tab:bis_exec}, vemos que
\begin{equation}
  |x^{(10)}-x^*| = 2,5\E-4,
\end{equation}
com $x^* = x_1 = 3\pi/4$. Observamos que este resultado é consistente com a estimativa do erro de truncamento \eqref{cap_eq1d_sec_bissec:eq:bis_est_trunc}, da qual temos
\begin{align}
