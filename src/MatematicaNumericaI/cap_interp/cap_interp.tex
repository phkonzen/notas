...
\begin{figure}[H]
  \centering
  \includegraphics[width=4in]{./cap_interp/dados/fig_poliInterp/fig.png}
  \caption{Polinômio interpolador referente ao Exemplo~\ref{cap_interp_sec_interpoli:ex:interpoli_intro}.}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=4in]{./cap_interp/dados/fig_poliLagrange/fig.png}
  \caption{Gráficos da função, dos pontos e do polinômio interpolador computado no Exemplo~\ref{cap_interp_sec_lagrange:ex:interpoli_aprox}.}
...
Observamos que o spline tem $4(n-1)$ coeficientes a determinar, enquanto que as condições acima nos fornecem $4n-6$ equações. Assim sendo, a determinação de \hl{um spline requer ainda duas condições de fechamento}. Conforme a escolha destas condições, diferentes splines cúbicos são computados.
...
\begin{ex}\label{cap_interp_sec_splines:ex:interp_spline_nak}
  ...
  \begin{equation}
    s(x) = \left\{\begin{array}{ll}
                          s_1(x) &, 0\leq x < \frac{\pi}{6}\\
                          s_2(x) &, \frac{\pi}{6} \leq x < \frac{\pi}{3},\\
                          s_3(x) &, \frac{\pi}{3} \leq x < \frac{\pi}{2}
           \end{array}\right.
  \end{equation}
  sendo suas partes
  \begin{equation}
    \begin{aligned}
      & s_1(x) = -0.11x^3 - 0.11x^2 - 0.11x, \\
      & s_2(x) = -0.07(x-\frac{\pi}{6})^3 - 0.24(x-\frac{\pi}{6})^2 - 0.42(x-\frac{\pi}{6}) + \frac{1}{2}, \\
      & s_3(x) = 1.02(x-\frac{\pi}{3})^3 + 0.86(x-\frac{\pi}{3})^2 + 0.51(x-\frac{\pi}{3}) + \frac{\sqrt{3}}{2}.
    \end{aligned}
  \end{equation}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[width=4in]{./cap_interp/dados/fig_CSNotAKnot/fig.png}
    \caption{Gráficos da função $f(x)=\sen(x)$ e do spline cúbico computado no Exemplo~\ref{cap_interp_sec_splines:ex:interp_spline_nak}.}
...
\right.
  ...
  \begin{equation}
    \begin{aligned}
      & s_1(x) = -0.16x^3 - 0.12x^2 - 0.04x \\
      & s_2(x) = -0.001(x-\frac{\pi}{6})^3 - 0.26(x-\frac{\pi}{6})^2 - 0.44(x-\frac{\pi}{6}) + \frac{1}{2}, \\
      & s_3(x) = 0.0(x-\frac{\pi}{3})^3 + 0.5(x-\frac{\pi}{3})^2 + 0.87(x-\frac{\pi}{3}) + \frac{\sqrt{3}}{2}.
    \end{aligned}
  \end{equation}

  \begin{figure}[H]
    \centering
    \includegraphics[width=4in]{./cap_interp/dados/fig_CSFixado/fig.png}
    \caption{Gráficos da função $f(x)=\sen(x)$ e do spline cúbico fixado computado no Exemplo~\ref{cap_interp_sec_splines:ex:interp_spline_fixado}.}
