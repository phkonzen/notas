...
\begin{resol}
  ...
  \item $\displaystyle \lim_{x\to -1} f(x)$
    ...
    \item $\displaystyle \lim_{x\to 1} f(x)$
      ...
      Aqui, para valores de $x$ suficientemente próximos de $x_0=1$ e a esquerda ($x<1$), vemos que os valores de $f(x)$ são próximos de $L=2$. Entretanto, para valores de $x$ suficientemente próximos de $x_0=1$ e a direira ($x>1$), temos que os valores de $f(x)$ são próximos de $L=1$. Ou seja, não é possível escolher um valor $L$ tal que $f(x)$ esteja arbitrariamente próxima ao tomarmos $x$ suficientemente próximo de $x_0=1$, pois $L$ dependerá de $x$ estar a esquerda ou a direita de do ponto $x_0 = 1$. Conluímos que este limite não existe, e escrevemos
      \begin{equation}
...
%     \end{equation}
%     \ifispython
%     No \href{https://www.sympy.org}{SymPy}, podemos computar este último exemplo com o código
% \begin{verbatim}
% f = lambda x: 2*x+x**2
% (f(1)-f(-1))/(1-(-1))
% \end{verbatim}
