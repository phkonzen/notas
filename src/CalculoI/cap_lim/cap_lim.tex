...
\end{equation}
e dizemos que o {\bf limite da função $f$ é $L$ quando $x$ tende a $x_0$}. Consultemos a Figura~\ref{fig:lim}.
  ...
  \item $\displaystyle \lim_{x\to 0} f(x) = 1 = f(0)$.
    ...
    \begin{center}
      \begin{tabular}{c|c}\toprule
        ...
        $0,0001$ & $1,0001$ \\
        $0,001$ & $1,001$ \\
        $0,01$ & $1,01$ \\\bottomrule
      \end{tabular}
    \end{center}

\ifispython
\begin{lstlisting}[style=input, caption={\python}, xrightmargin=0.35in]
from sympy.abc import x
from sympy import limit
limit((x**2-1)*(x-2)/((x-1)*(x-2)), x, 0)
\end{lstlisting}
%
\begin{lstlisting}[style=output, xrightmargin=0.35in]
1
\end{lstlisting}  
\fi
...
\begin{equation}
  \lim_{x\to x_0} k = k,
\end{equation}
seja qual for a constante $k$. Consultemos a Figura~\ref{fig:lim_funk}.

\begin{figure}[H]
  \centering
  \includegraphics[width=2.75in]{./cap_lim/dados/fig_lim_funk/fig.png}
...
\begin{equation}
  \lim_{x\to a} x = a,
\end{equation}
seja qual for o ponto $a$. Consultemos a Figura~\ref{fig:lim_funid}.

\begin{figure}[H]
  \centering
  \includegraphics[width=2.75in]{./cap_lim/dados/fig_lim_funid/fig.png}
...
\begin{exeresol}
  ...
  \begin{center}
    \includegraphics[width=2.75in]{./cap_lim/dados/fig_exeresol_nocaolim/fig.png}
  \end{center}
...
\begin{exer}\label{exer:limgraf}
  ...
  \begin{center}
    \includegraphics[width=2.75in]{./cap_lim/dados/fig_exer_limgraf/fig.png}
  \end{center}
...
\ifispython
\begin{lstlisting}[style=input, caption={\python}, xrightmargin=0.19in]
from sympy.abc import x
from sympy import limit
limit(2*x, x, -1)  
\end{lstlisting}
%
\begin{lstlisting}[style=output, xrightmargin=0.19in]]
-2
\end{lstlisting}
\fi
...
\begin{prop}[Limites de polinômios]\label{prop:lim_poli}
  Se
  \begin{equation}
...
\begin{prop}[Limite de funções racionais]
...
\ifispython
\begin{lstlisting}[style=input, caption={\python}, firstnumber=9]
from sympy import limit
limit(p/q, x, 1)  
\end{lstlisting}
%
...
\begin{lstlisting}{style=input, caption={\python}}
from sympy.abc import x
from sympy import limit, Function, Eq, solve
f = Function('f')
L = limit(f(x), x, 2)
eq = Eq(limit((f(x)-2)/(x+2), x, 2), 1)
solve(eq, L)
\end{lstlisting}
%
\begin{lstlisting}{style=output}
...
\begin{exemplo}\label{ex:lim_absx}
  Vamos calcular
  \begin{equation}
    \lim_{x\to 0^-} |x|.
  \end{equation}
...
\begin{obs}[Regras para o cálculo de limites no infinito]\label{obs:lim_regras_xinf}
...
\begin{ex}[Função exponencial natural]\label{ex:lim_exp_x-inf}
  \begin{equation}
...
\begin{ex}[Continuidade da função valor absoluto]\label{ex:lim_fabs_cont}
...
\begin{teo}[Teorema do valor intermediário]\label{teo:valorintermediario}
...
\begin{teo}[Teorema do confronto]\label{teo:confronto}
