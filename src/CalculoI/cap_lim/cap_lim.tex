...
Dizemos que o limite de $f(x)$ é $L$ quando $x$ tende a $-\infty$, se os valores de $f(x)$ são arbitráriamente próximos de $L$ para valores de $x$ suficientemente pequenos. Neste caso, escrevemos
\begin{equation}
...
\begin{ex}\normalfont{(Limites no infinito de funções racionais)}\label{ex:liminf_funracio}
...
\begin{ex}\normalfont{(Função exponencial natual)}
  \begin{equation}
    \lim_{x\to -\infty} e^x = 0,
  \end{equation}
  donde temos que $y=0$ é uma assíntota horizontal da função exponencial natual.
...
Além de assíntotas horizontais e verticais, gráficos de funções podem ter assíntotal oblíquas. Isto ocorre, particularmente, para funções racionais cujo grau do numerador é maior que o do denominador.
...
\end{equation}
Entretanto, como é o caso assima, em muitos casos podemos concluir mais sobre a tendência da função. Por exemplo, no caso acima, quando $x\to 0$ os valores de $f(x)$ crescem arbitráriamente, i.e. $f(x)\to \infty$.

Mais precisamente, dizemos que o limite de uma dada função $f(x)$ é infinito quando $x$ tende a um número $a$, quando $f(x)$ torna-se arbitráriamente grande para valores de $x$ suficientemente próximos de $a$. Neste caso, escrevemos
\begin{equation}
...
\begin{ex}
  ...
  \end{equation}
  Observe que $1/|x|$ torna-se arbitráriamente grande para valores de $x$ suficientemente próximos a $a$.
\end{ex}
...
\begin{ex}
  ...
  \item A função $\displaystyle f(x) = \frac{x^3 - 2x + 1}{2 - 3x^3}$ não está definida para valores de $x$ tais que seu denominador se anule, i.e.
    ...
    \begin{equation}
      \lim_{x\to \sqrt[3]{\frac{2}{3}}^+} = \infty.
    \end{equation}
    Coms isso, dizemos que $x = \sqrt[3]{2/3}$ é uma assíntotal vertical do gráfico desta função. Veja a Figura \ref{fig:ex_ass}.
  \end{itemize}
\end{ex}
