...
\end{lstlisting}
  ...
  \begin{figure}[H]
    ...
    \caption{Comparação da autograd da MLP com a derivada exata $f'(x)=\cos(x)$ para o Exemplo \ref{cap_mlp_sec_autograd:ex:fun1d}.}
    \label{cap_mlp_sec_autograd:fig:fun1d}
  \end{figure}

  Uma vez treinada, nossa MLP é uma aproximadora da função seno, i.e. $\tilde{y} \approx \sen(x)$. Usando de autograd podemos computar $\tilde{y}' \approx \cos(x)$. O código abaixo, computa $d\tilde{y}/dx$ a partir da rede e produz o gráfico da figura acima.
