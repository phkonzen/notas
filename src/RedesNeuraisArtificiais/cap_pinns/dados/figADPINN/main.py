...
# exact solution
def exact(t):
    return torch.exp(t) - 0.5*(torch.sin(t) + torch.cos(t))
...
ns = 100
tt = torch.linspace(t0, tf, ns+1).reshape(-1,1)
ye = exact(tt)
ym = model(tt).detach()

## Explicit Euler
yee = torch.empty_like(ts)
yee[0] = y0
for k in range(ns):
    yee[k+1] = yee[k] + ht*g(ts[k], yee[k])

fig = plt.figure(dpi=300)
ax = fig.add_subplot()

ax.plot(tt, ye, label='exact')
ax.plot(ts, yee, ls='--', marker='o', label='Euler')
ax.plot(tt, ym, ls='--', label='model')
