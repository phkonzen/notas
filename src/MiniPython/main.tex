...
\item \hl{por execução de um código {\lstinline+arqnome.py+} em um console/notebook {\python}};
\item \hl{por execução de um cógido {\lstinline+arqnome.py+} em um terminal do sistema operacional}.
\end{itemize}
...
>>> y = x * 2.0
\end{lstlisting}
Na primeira instrução, a variável \lstinline+x+ recebe o valor inteiro \lstinline+1+ e, então, é armazenado na memória do computador como um objeto da \hl{classe {\lstinline+int+}} (número inteiro). Na segunda instrução, \lstinline+y+ recebe o valor decimal $2.0$ (resultado de $1\times 2.0$) e é armazenado como um objeto da \hl{classe {\lstinline+float+}} (ponto flutuante de 64-{\it bits}). Podemos verificar isso, com as seguintes instruções
\begin{lstlisting}
>>> print(x, y)
1 2.0
>>> print(type(x), type(y))
...
\item[]\hl{{\lstinline+**+} \emph{: potenciação}}
...
\begin{itemize}
\item[]\hl{{\lstinline+and+} \emph{: {\lstinline+e+} lógico}}
\item[]\hl{{\lstinline+or+} \emph{: {\lstinline+ou+} lógico}}
\item[]\hl{{\lstinline+not+} \emph{: {\lstinline+não+} lógico}}
\end{itemize}

\begin{ex}\normalfont{(\hl{Tabela Booleana do {\lstinline+and+}}.)}
