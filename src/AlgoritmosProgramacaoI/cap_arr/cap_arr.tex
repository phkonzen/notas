...
\begin{obs}[\hl{Métodos de Reordenamento}]
...
\begin{obs}[\hl{Concatenação de Arranjos}]
...
\begin{exer}
  ...
  \begin{enumerate}[a)]
    ...
    % c)
    \item O atributo \underline{\phantom{\texttt{.size}}} é o número total de objetos de um {\PYTHONnumpyDOTarray}.
    % d)
...
\begin{exer}
  ...
  \begin{enumerate}[a)]
    % a)
    \item Usualmente, matrizes são computacionalmente modeladas como \underline{\phantom{\texttt{arrays}}} multidimensionais.
    % b)
    \item A operação de multiplicação matricial entre dois \texttt{arrays} pode ser feita com o operador \underline{\phantom{\texttt{@}}} ou com o método {\PYTHONnumpyDOTmatmul}.
    % c)
    \item A transposta de um {\PYTHONnumpyDOTarray} bidimensional pode ser obtida com o atributo \underline{\phantom{\texttt{.T}}} ou o método {\PYTHONnumpyDOTtranspose}.
    % d)
