...
\begin{exer}\label{cap_progest_sec_ramifica:exer:entre_curvas}
  ...
  % \begin{figure}[H]
  %   \centering
  %   \includegraphics[width=3in]{./cap_progest/dados/fig_exer_entre_curvas/fig.png}
  % \end{figure}
...
\begin{ex}[\hl{Somatório com \texttt{while}}]\label{cap_progest_ec_repete:ex:while_soma_num}
...
\begin{ex}[\hl{(Aproximando a $\sqrt{x}$)}]\label{cap_progest_sec_repete:ex:heron}
...
\begin{ex}[\hl{Somatório com \texttt{for}}]\label{cap_progest_ec_repete:ex:for_soma_num}
