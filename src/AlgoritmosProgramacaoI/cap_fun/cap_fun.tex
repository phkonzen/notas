...
\hl{O módulo {\PYTHONmath} contém constantes e funções matemáticas elementares de números reais}. Para \hlemph{importar o módulo} em nosso código, podemos usar a instrução \hl{{\PYTHONimport}}
...
\begin{exer}
  ...
  \begin{enumerate}[a)]
    ...
    % c)
    \item A função {\PYTHONprint} permite a impressão de uma \underline{\phantom{\texttt{string}}}.
    % d)
...
\begin{exer}
  ...
  \begin{enumerate}[a)]
    ...
    % d)
    \item Parâmetros de entrada arbitrários podem ser definidos usando \underline{\phantom{\texttt{tuples}}} ou \underline{\phantom{\texttt{dicts}}}.
  \end{enumerate}
\end{exer}
\begin{resp}
