...
\hl{O módulo \texttt{math} fornece acesso a constantes e funções matemáticas elementares para números reais}. Para \hlemph{importar o módulo} em nosso código, podemos usar a instrução \hl{\texttt{import}}. Por exemplo,
...
\begin{exer}
  ...
  \begin{figure}[H]
    \centering
    \includegraphics[max width = 0.9\textwidth, height=3.75cm]{./cap_fun/dados/fig_leiDosCossenos/fig.png}
  \end{figure}
...
\end{exer}
\begin{resp}
  Dica: O módulo \lstinline+random+ fornece a função \href{https://docs.python.org/3/library/random.html\#random.randint}{\lstinline+random.randint(a, b)+}\endnote{Consulte na \textit{web} \href{https://docs.python.org/3/library/random.html\#module-random}{The Python Standard Library: Numeric and Mathematical Moduels: random}.} que retorna um inteiro $a \leq x \leq b$.
\end{resp}
