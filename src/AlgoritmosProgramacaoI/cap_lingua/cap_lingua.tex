...
\begin{exer}
  ...
  \begin{enumerate}[a)]
    ...
    % b)
    \item O comando \underline{\phantom{\texttt{x in a}}} retorna {\PYTHONFalse}, se \texttt{x} não é um item de {\PYTHONset} \texttt{a}.
    % c)
    \item O comando \underline{\phantom{\texttt{a != b}}} retorna {\PYTHONtrue}, se os \texttt{sets} \texttt{a} e \texttt{b} são diferentes.
    % d)
    \item Se todos os itens do {\PYTHONset} \texttt{a} pertencem ao {\PYTHONset} \texttt{b}, então o comando \texttt{a <= b} retorna \underline{\phantom{{\PYTHONTrue}}}.
    % e)
    \item Se \texttt{x} é um item do {\PYTHONset} \texttt{a} ou do \texttt{b}, então o comando \underline{\phantom{\texttt{x in a | b}}} retorna {\PYTHONTrue}.
  \end{enumerate}
\end{exer}
\begin{resp}
  a) coleção; b) \texttt{x in a}; c) \texttt{a != b}; d) {\PYTHONTrue}; e) {\PYTHONTrue}
\end{resp}
