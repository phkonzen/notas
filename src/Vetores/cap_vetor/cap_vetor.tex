...
\begin{exer}
  Mostre que segmentos terem a mesma norma é uma:
  \begin{enumerate}[a)]
...
\end{exer}
\begin{resp}
  a) Por óbvio, que $AB$ tem a mesma norma que si próprio. b) Se $AB$ tem a mesma norma de $CD$, $|AB| = |CD|$, então é dizer que $CD$ tem a mesma norma de $AB$. c) Se $|AB| = |CD|$ e $|CD| = |EF|$, então $|AB| = |EF|$. d) Por definição, segue dos itens $a)$, $b)$ e $c)$.
\end{resp}
...
\end{proposicao}
\begin{demonstracao}
  Primeiramente, vamos mostrar a implicação. Por hipótese, temos que $|\vec{u}| = 0$. Seja, $\overrightarrow{AB}$ uma representação de $\vec{u}$. Então, por definição da norma de vetor, $|\vec{u}| := \left|\overrightarrow{AB}\right| = 0$. Logo, $AB$ é um segmento nulo e, portanto, $\vec{u} = \vec{0}$.

  Agora, mostramos a recíproca, i.e., se $\vec{v} = \vec{0}$, então $|\vec{u}| = 0$. Como $\vec{u} = \vec{0}$, temos que $\vec{u}$ pode ser representado por qualquer segmento orientado $\overrightarrow{AA}$. Temos que $\left|\overrightarrow{AA}\right| = 0$ e, portanto, $\vec{u} := \left|\overrightarrow{AA}\right| = 0$.
\end{demonstracao}

\hl{Dois \textbf{vetores} são ditos \textbf{paralelos} quando quaisquer de suas representações têm a mesma direção}. De forma análoga, definem-se \textbf{vetores coplanares}, \textbf{vetores não coplanares}, \textbf{vetores ortogonais}, além de conceitos como \textbf{ângulo entre dois vetores}, etc.

\begin{ex}
  Na Figura~\ref{cap_vetor_sec_vetor:fig:vetores_pararel_perp}, temos $\vec{u}$ vetor paralelo a $\vec{v}$, enquanto que $\vec{x}$ é perpendicular a $\vec{y}$.
...
\begin{demonstracao}
  ...
  Agora, vamos \emph{mostrar a unicidade}, i.e. que se $A$ e $B$ são pontos tais que $\vec{u}=\overrightarrow{OA}=\overrightarrow{OB}$, então $A$ e $B$ são coincidentes. \emph{Por contradição}, se $A$ e $B$ não forem coincidentes, então $\overrightarrow{OA}$ e $\overrightarrow{OB}$ têm sentidos opostos ou $\left|\overrightarrow{OA}\right|\neq\left|\overrightarrow{OB}\right|$. Em qualquer um destes casos, $\overrightarrow{OA}$ e $\overrightarrow{OB}$ não são equipolentes. Logo, $A$ e $B$ são necessariamente coincidentes.
\end{demonstracao}
...
\begin{exer}
  Mostra que uma reta fica unicamente determinada por um ponto $O$ e um vetor não nulo $\vec{u}$.
\end{exer}
\begin{resp}
