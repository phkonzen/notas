...
\begin{obs}\normalfont{(Regra do paralelogramo)}
  Sejam vetores não nulos $\vec{u} = \overrightarrow{AB}$ e $\vec{v} = \overrightarrow{AC}$. Seja, ainda, $D$ o vértice oposto a $A$ no paralelogramo determinado pelos lados formados pelos segmentos $AB$ e $AC$. Então, temos $\vec{u} + \vec{v} = \overrightarrow{AD}$ e $\vec{u}-\vec{v} = \overrightarrow{CB}$. Veja a Figura \ref{fig:regrapara}.
