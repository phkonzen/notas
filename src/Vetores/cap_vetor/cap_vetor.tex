...
\begin{itemize}
  \item \hlemph{Existência}
  
  Seja dado um vetor e sua representação $\vec{u} = \overrightarrow{AB}$. O segmento $\overrightarrow{BA}$ tem o mesmo comprimento, a mesma direção e sentido oposto a $\overrightarrow{AB}$. Portanto, escolhendo $-\vec{u}$ o vetor que admite representação $\overrightarrow{BA}$, temos que $-\vec{u}$ é vetor oposto a $\vec{u}$. Consulte a Figura~\ref{cap_vetor_sec_op:fig:vetor_oposto}.

  \item \hl{$\pmb{\left\|\vec{u}\right\| = \left\|-\vec{u}\right\|}$}
  
  De fato, seja o vetor e sua representação $\vec{u} = \overrightarrow{AB}$, então
  \begin{align}
    \left\|\vec{u}\right\| &:= \left\|AB\right\|\\
    &= \left\|BA\right\|\\
    &=: \left\|-\vec{u}\right\|
  \end{align}
\end{itemize}
