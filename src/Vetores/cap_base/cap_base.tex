...
\subsection{Dois Vetores no Espaço}
...
Reciprocamente, \emph{se $\vec{u}$, $\vec{v}$ e $\vec{w}$ são coplanares, então $\vec{u}$, $\vec{v}$ e $\vec{w}$ são l.d..} Consulte a Figura~\ref{cap_base_sec_deplinear:fig:vetores_coplanares_ld}.
