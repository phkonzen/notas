...
# pragma omp parallel
\end{verbatim}
i.e., entre as linhas 9 e 15. Em paralelo, cada {\it thread} registra seu número de identificação na variável interira \lstinline+id+, veja a linha 12. Na linha 14, escrevem a saudação, identificando-se.
...
\begin{exeresol}
  O número de instâncias de processamento pode ser alterado pela variável do sistema \verb+OMP_NUM_THREADS+. Altere o número de {\it threads} para 2 e execute o Código \ref{lst:ola}.
\end{exeresol}
\begin{resol}
...
\begin{exer}
  Defina um número de {\it threads} maior do que o disponível em sua máquina. Então, rode o Código \ref{lst:ola} e analise a saída. O que você observa?
\end{exer}

\begin{exer}
  Modifique o Código \ref{lst:ola} de forma que cada {\it thread} escreva na tela ``Processo ID de NP, olá!'', onde ID é a identificação do {\it thread} e NP é o número total de {\it threads} disponíveis. O número total de {\it threads} pode ser obtido com a função {\omp}
...
\begin{exer}\label{exer:cc_exer_dot}
  Escreva um código MP para computar o produto escalar entre dois vetores de $n$ elementos. Inicialize todos os elementos com números randômicos em ponto flutuante. Ainda, o código deve ser escrito para um número arbitrário $m\geq 1$ de instâncias de processamento.
\end{exer}
