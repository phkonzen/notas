...
\end{equation}
Observamos que $\pi f$ é igual a $f$ nos nodos $x_0$ e $x_1$. Agora, vamos buscar medir o erro de interpolação, i.e. $f - \pi f$. Para tanto, podemos usar a norma $L^2$ definida por
\begin{equation}
...
\begin{prop}\normalfont(Erro da interpolação linear)\label{prop:pif}
...
\end{equation}
Neste caso, temos o seguinte resultado para o erro de interpolação.
...
\end{prop}
\begin{dem}
  Ambas desigualdades seguem da desigualdade triangular e da Proposição~\ref{prop:pif}. Por exemplo, para a primeira desigualdade, temos
  \begin{align}
