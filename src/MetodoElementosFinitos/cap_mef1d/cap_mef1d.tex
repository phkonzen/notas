...
\section{Aplicação: Equação de Fisher}\label{cap_mef1d_sec_fisher}
\badge[yellow]{Em construção}

Consideramos a \emph{equação de Fisher}{\fisher} com dadas condição inicial e condições de contorno de Neumann{\neumann}
\begin{subequations}\label{cap_mef1d_sec_fisher:eq:prob}
  \begin{align}
...
Consideramos os $n_t+1$ tempos discretos $t^{(k)} = k h_t$, passo no tempo $h_t = t_f/n_t$, $k = 0, 1, 2, \dotsc, n_t$. Seguindo esquema $\theta$ denotando $u^{(k)} \approx u\left(t^{(k)}, x\right)$, o problema \eqref{cap_mef1d_sec_fisher:eq:prob} pode ser aproximado pela iteração
\begin{subequations}\label{cap_mef1d_sec_fisher:eq:theta}
  \begin{align}
...
Por simplificação da notação, vamos suprimir o super-índice $k$, denotando $u^{(k+1)} := u$, $u^{(k)} = u^0$. Com isso e rearranjando os termos, cada iteração \eqref{cap_mef1d_sec_fisher:eq:theta} se resume ao seguinte problema de valores de contorno
\begin{subequations}\label{cap_mef1d_sec_fisher:eq:theta-iter}
  \begin{align}
...
A formulação fraca do problema \eqref{cap_mef1d_sec_fisher:eq:theta-iter} consiste em: encontrar $u\in V := H^1[0,1]$ tal que
\begin{equation}
...
\end{equation}
Com isso, a formulação de elementos finitos do problema \eqref{cap_mef1d_sec_fisher:eq:theta-iter} consiste em: encontrar $u_h\in V_h$ tal que
\begin{equation}\label{cap_mp_sec_fisher:eq:fem}
  F(u_h; v) = 0, ~\forall v_h\in V_h.
\end{equation}

\begin{obs}
  O problema \eqref{cap_mp_sec_fisher:eq:fem} consiste em um sistema de equações não-lineares.
\end{obs}
