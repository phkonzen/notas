...
\lstinputlisting[caption=perceptron.py, label=cap_perceptron_sec_unit:cod:perceptron]{./cap_perceptron/dados/py_perceptron/main.py}
...
Na Subseção \ref{cap_perceptron_ssec_classic}, treinamos um Perceptron para o problema de classificação do e-lógico. Usamos $f(x) = \sign(x)$ como função de ativação. Ocorre, que para o Método do Gradiente, esta função de ativação não é apropriada, pois $f'(x) \equiv 0$ para $x\neq 0$. Aqui, vamos usar
\begin{equation}
  f(x) = \tanh(x).
\end{equation}
...
O Método do Gradiente Estocástico é um variação do método anterior. A ideia é atualizar os parâmetros do modelo com base no gradiente do erro de cada amostra. A estocasticidade é obtida da randomização com que as amostras são escolhidas a cada época. O algoritmos consiste no seguinte:
\begin{enumerate}[1.]
...
\begin{exer}\label{cap_perceptron_sec_train:exer:prob_regr}
  Use o Método do Gradiente para treinar um Perceptron para o problema de classificação estudado na Subseção \ref{cap_perceptron_ssec_regr}.
\end{exer}

\begin{exer}
  Refaça o Exercício \ref{cap_perceptron_sec_train:exer:prob_regr} usando o Método do Gradiente Estocástico como otimizador.
\end{exer}
