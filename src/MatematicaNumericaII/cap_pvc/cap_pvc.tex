...
\begin{subequations}
  \begin{align}
    a_{i,i} &= (\phi_i, \phi_i)_2\\
            ...
            &= \int_{x_{i-1}}^{x_i} \frac{(x-x_{i-1})^2}{h^2}\,dx\\
            &+ \int_{x_{i}}^{x_{i+1}} \frac{(x_{i+1}-x_{i})^2}{h^2}\,dx\\
            &= \frac{2}{h}, ~i=1, 2, \dotsc, n-1,
  \end{align}
\end{subequations}
\begin{subequations}
  \begin{align}
    a_{i,i+1} &= (\phi_{i+1}, \phi_i)_2\\
            &= \int_a^b \phi_{i+1}\phi_i\,dx\\
            &= \int_{x_{i}}^{x_{i+1}}\frac{(x_{i+1}-x)}{h}\frac{(x - x_{i+1})}{h}\,dx\\
            &= -\frac{1}{h}, ~i=1, 2, \dotsc, n-2,
  \end{align}
\end{subequations}
\begin{subequations}
  \begin{align}
    a_{i-1,i} &= (\phi_{i-1}, \phi_i)_2\\
            &= \int_a^b \phi_{i-1}\phi_i\,dx\\
            &= \int_{x_{i-1}}^{x_{i}}\frac{(x_i-x)}{h}\frac{(x - x_{i})}{h}\,dx\\
