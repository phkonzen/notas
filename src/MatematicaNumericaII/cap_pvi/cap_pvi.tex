...
\begin{obs}\normalfont{(Consistência do Método de Euler.)}
  ...
  \begin{align}
    ...
    &\text{}\quad = \lim_{h\to 0} \left(\frac{y(t+h)-y(t)}{h} - f\left(t,y(t)\right)\right)\\
    &\text{}\quad = y'(t) - f\left(t, y(t)\right) = 0.
  \end{align}
\end{obs}
...
\begin{align}
  ...
  &\text{}\quad = f_t(t, y) + f_y(t, y)y'\\
  &\text{}\quad = f_t(t, y) + f_y(t, y)f(t, y).
\end{align}
...
\begin{align}
  & \tau(t, y; h) := \Delta(t, y; h)-\Phi(t, y; h)\\
  &\text{}\quad = \Delta(t, y; h) - f(t, y)\\
  &\text{}\quad = \frac{h}{2}[f_t(t, y) + f_y(t, y)f(t, y)] + O(h^2)\\
  &\text{}\quad = O(h).
\end{align}
...
\begin{dem}
  ...
  \begin{align}
    ...
    & \text{}\quad = (1+\delta)^2\left|\xi^{(0)}\right| + (1+\delta)B + B\\
    & \text{}\qquad \vdots\\
    & \left|\xi^{(k)}\right| \leq (1 + \delta)^k\left|\xi^{(0)}\right| + B\sum_{k=0}^{k-1}(1+\delta)^k\\
    & \text{}\quad = (1 + \delta)^k\left|\xi^{(0)}\right| + B\frac{(1+\delta)^k-1}{\delta}.
  \end{align}
...
\begin{dem}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      & \text{}\quad + h\left[f\left(t^{(k)}, y^{(k)}\right) - f\left(t^{(k)}, y\left(t^{(k)}\right)\right)\right] - \frac{h^2}{2}y''\left(\xi^{(k)}\right)
    \end{aligned}
  \end{equation}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      & \text{}\quad + hL\left|y^{(k)} - y\left(t^{(k)}\right)\right| + \frac{h^2}{2}\left|y''\left(\xi^{(k)}\right)\right|
    \end{aligned}
  \end{equation}
  Ou, ainda,
  \begin{equation}
...
\begin{equation}\label{cap_pvi_sec_euler:eq:euler_errarr_est}\hleq
  \begin{aligned}
    ...
    & \text{}\quad + |\delta_0|e^{L\left(t^{(k)}-t_0\right)},
\end{aligned}
\end{equation}
...
\begin{exer}
  ...
  \begin{subequations}
    ...
    \end{align}
  \end{subequations}
  tem solução analítica $y(t) = \sen(\pi t)\cos(\pi t)$. Compute a aproximação $\tilde{y}(1.5; h) \approx y(1.5)$ pelo método de Euler com passo $h=10^{-1}$ e forneça o erro $e(1.5; h) := \left|\tilde{y}(1.5; h) - y(1.5)\right|$.
\end{exer}
\begin{resp}
...
\begin{equation}
  \begin{aligned}
    ...
    & \text{}\quad + \cdots + \frac{h^m}{m!}\frac{d^m y}{d t^m}(t) + \frac{h^{m+1}}{(m+1)!}\frac{d^{m+1} y}{d t^{m+1}}(\xi),
  \end{aligned}
\end{equation}
donde
\begin{equation}\label{cap_pvi_sec_taylor:eq:y_poli_taylor}
  \begin{aligned}
    ...
    & \text{}\quad + \cdots + \frac{h^m}{m!}\frac{d^{m-1} f}{d t^{m-1}}(t, y) \\
    & \text{}\quad + \frac{h^{m+1}}{(m+1)!}\frac{d^{m} f}{d t^{m}}\left(\xi, y(\xi)\right)
  \end{aligned}
\end{equation}
e, portanto
...
\begin{equation}\hleq
  \begin{aligned}
    ...
    & \text{}\quad + \cdots + \frac{h^{m-1}}{m!}\frac{d^{m-1} f}{d t^{m-1}}\left(t^{(k)}, y^{(k)}\right)
\end{aligned}
\end{equation}
...
\begin{dem}
  ...
  \begin{align}
    ...
    & \text{}\quad = \tilde{e}^{(k)} + h\left[\tilde{\Phi}\left(t^{(k)}, \tilde{y}^{(k)}; h\right) - \tilde{\Phi}\left(t^{(k)}, y\left(t^{(k)}\right); h\right)\right]\\
    & \text{}\qquad + h\left[\tilde{\Phi}\left(t^{(k)}, y\left(t^{(k)}\right); h\right) - \Delta\left(t^{(k)}, y\left(t^{(k)}\right); h\right)\right].\label{cap_pvi_sec_taylor:eq:aux2}
  \end{align}
...
\begin{align}
  ...
  &\text{}\quad \hleq{= f(t, y(t)) + \frac{h}{2}\left[f_t(t, y)\right.}\nonumber\\
  &\text{}\quad \hleq{\left. + f_y(t, y)f(t, y)\right] + O(h^2)}\label{cap_pvi_sec_rk:eq:pvi_delta_aux2}.
\end{align}
...
\begin{equation}\label{cap_pvi_sec_rk:eq:pvi_phi_aux2}\hleq
  \begin{aligned}
    ...
    & \text{}\quad +\beta_{2,1}f_y(t,y)f(t,y)) + O(h^2).
  \end{aligned}
\end{equation}
...
\begin{equation}\hleq
  \begin{aligned}
    ...
    & \text{}\quad \left. + f\left(t^{(k)}+h, y^{(k)}+hf\left(t^{(k)},y^{(k)}\right)\right)\right].
  \end{aligned}
\end{equation}
...
\begin{align}
  & \tau(t, y; h) := \Delta(t,y; h) - \Phi(t, y; h) \\
  & \text{}\quad = \underbrace{\frac{y(t+h) - y(t)}{h}}_{:= \Delta} - \underbrace{f(t+h, y(t+h); h)}_{:= \Phi}\\
  & \text{}\quad = O(h).
\end{align}
...
\begin{align}
  ...
  & \text{}\quad = \left|\left(e^{\lambda h}\right)^k - (1 + h\lambda)^k\right||y_0|
\end{align}
...
\begin{resp}
  \begin{align}
    ...
    & \text{}\quad = y_0\lim_{h\to 0^+} \left(1 - h\lambda\right)^{-t^{(k)}/h}\\
    & \text{}\quad = y_0 \lim_{h\to 0^+} \left(1 - h\lambda\right)^{-k}\\
    & \text{}\quad = \lim_{h\to 0^+} y^{(k)}.
  \end{align}
\end{resp}
...
\begin{equation}\hleq\label{cap_pvi_sec_passo_mult:eq:iter_am2}
  \begin{aligned}
    ...
    & \text{}\quad + 8f\left(t^{(k)}, y\left(t^{(k)}\right)\right) \\
    & \text{}\quad \left.  - f\left(t^{(k-1)}, y\left(t^{(k-1)}\right)\right)\right]
  \end{aligned}
\end{equation}
...
\begin{equation}\label{cap_pvi_sec_passo_mult:eq:iter_am4}\hleq
  \begin{aligned}
    ...
    & \text{}\quad \left. + 646f\left(t^{(k)},y^{(k)}\right) - 264f\left(t^{(k-1)},y^{(k-1)}\right) \right. \\
    & \text{}\quad \left. + 106f\left(t^{(k-2)},y^{(k-2)}\right) - 19f\left(t^{(k-3)},y^{(k-3)}\right)\right].
  \end{aligned}
\end{equation}
...
\begin{exer}
  ...
  \begin{equation}
    \begin{aligned}
      ...
      & \text{}\quad \left. - 16f(t^{(k-1)},y^{(k-1)}) + 5f(t^{(k-2)},y^{(k-2)})\right],
    \end{aligned}
  \end{equation}
...
\begin{exer}
  ...
  \begin{equation}
    ...
    & \text{}\quad \left. + 19f\left(t^{(k)},y^{(k)}\right) - 5f\left(t^{(k-1)},y^{(k-1)}\right) \right. \\
    & \text{}\quad \left. + f\left(t^{(k-2)},y^{(k-2)}\right)\right].      
  \end{aligned}
  \end{equation}
