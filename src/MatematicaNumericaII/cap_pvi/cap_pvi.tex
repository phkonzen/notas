...
\begin{equation}
  \begin{aligned}
    ...
    &+ \cdots + \frac{h^m}{m!}\frac{d^{m-1} f}{d t^{m-1}}\left(t^{(k)}, y^{(k)}\right) \\
    & + \frac{h^{m+1}}{(m+1)!}\frac{d^{m} f}{d t^{m}}\left(\xi^{(k)}, y\left(\xi^{(k)}\right)\right).
  \end{aligned}
\end{equation}
...
\begin{exer}
  ...
  \begin{subequations}\label{cap_pvi_sec_taylor:eq:pvi_exer_0}
    ...
    \end{align}
  \end{subequations}
  A solução analítica é $y(t) = \cos(t) - \sin(t)$. Faça testes numéricos com $h=10^{-1}$, $10^{-2}$, $10^{-3}$ e $10^{-4}$, observe os resultados obtidos e o erro $\varepsilon := |\tilde{y}(1) - y(1)|$, onde $\tilde{y}$ corresponde a solução numérica. O erro tem o comportamento esperado? Justifique sua resposta.
\end{exer}
\begin{resp}
  \begin{tabular}{c|c|c}
