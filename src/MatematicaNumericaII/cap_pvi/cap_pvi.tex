...
Seja um \emph{Problema de Valor Inicial} (PVI)
\begin{subequations}\hleq
  \begin{align}
...
\begin{equation}\label{cap_pvi_sec_passo_mult:eq:tfc}
...
\hl{Métodos de Adams-Bashforth são métodos explícitos de passo múltiplo} obtidos ao escolhermos $j=0$ e $i=1$ em \eqref{cap_pvi_sec_passo_mult:eq:tfc}, i.e.
\begin{equation}
...
\begin{equation}\hleq
  \begin{aligned}
    ...
    &\qquad\qquad\left. - 59f(t^{(i-1)},y^{(i-1)}) + 37f(t^{(i-2)},y^{(i-2)}) \right. \\
    &\qquad\qquad\left. -9f(t^{(i-3)},y^{(i-3)})\right],
  \end{aligned}
\end{equation}
...
\subsubsection{Método de Admans-Moulton de Ordem 4}

[[tag:construcao]]

\subsection{Método Preditor-Corretor}
