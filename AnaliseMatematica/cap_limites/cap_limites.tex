...
\begin{corol}\normalfont{(Permanência do sinal)}\label{corol:limite_permanência_do_sinal}
...
\begin{teo}\normalfont{(Operações com limites)}\label{teo:operações_com_limites}
...
\begin{corol}\label{corol:limites_de_funções_e_sequencias}
...
\begin{dem}
  \begin{enumerate}[a)]
  \item A suficiência segue do critério de convergência de Cauchy para sequências e do Corolário~\ref{corol:limites_de_funções_e_sequencias}.
...
\begin{exer}
  Demonstre o Teorema~\ref{teo:operações_com_limites} como um corolário do Teorema~\ref{teo:limites_e_sequencias}.
\end{exer}
\begin{resp}
